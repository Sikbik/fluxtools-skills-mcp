...
- Persisted CLI profile state now carries per-base-url auth and enterprise-key caches (`zelidauthByBaseUrl` and `enterpriseKeyByBaseUrl`) so gateway pinning and explicit base-URL switches can adopt matching saved credentials. Scrutiny for `m3-auth-node` found that uncached target switches still retain the previous node's credentials, so do not assume an uncached base URL clears auth/key state until that follow-up fix lands.
