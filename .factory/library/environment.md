...
- Persisted CLI profile state now carries per-base-url auth and enterprise-key caches (`zelidauthByBaseUrl` and `enterpriseKeyByBaseUrl`) so gateway pinning and explicit base-URL switches can adopt matching saved credentials. When a normalized target has no cache entry, `flux node use-base-url` / `use-gateway` clear active auth and enterprise-key state instead of carrying credentials across nodes.
